...
async function searchRestaurant(){
  ...
  var proteinIcons={chicken:'🍗',steak:'🥩',fish:'🐟'};
...
function renderFoodCard(item,isBest,calBudget,isToday,restaurant){
  ...
  var pIcons={chicken:'🍗',steak:'🥩',fish:'🐟'};
  var pIcon=item.protein?pIcons[item.protein]||'':'';
  ...
  return '<div style="'+cardBg+';border-radius:14px;padding:16px;margin-bottom:12px;box-shadow:0 8px 32px rgba(0,0,0,.4)">'+
    ...
    '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">'+
      '<div style="font-size:.98rem;font-weight:700;color:var(--t1);font-family:var(--font-display)">'+(pIcon?pIcon+' ':'')+item.name+'</div>'+
